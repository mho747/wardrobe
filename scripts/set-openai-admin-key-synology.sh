...
printf '%s' 'OpenAI Admin API key for Costs (hidden): ' > "$TTY"
stty -echo < "$TTY"
...
while IFS= read -r line || [ -n "$line" ]; do
  case "$line" in
    OPENAI_ADMIN_API_KEY=*) printf 'OPENAI_ADMIN_API_KEY=%s\n' "$admin_key" >> "$replacement"; found=1 ;;
    *) printf '%s\n' "$line" >> "$replacement" ;;
  esac
done < "$ENV_FILE"
...
rm -f "$rollback"
rollback=
printf '%s\n' 'OpenAI Admin key stored locally. Wardrobe is healthy; no OpenAI request was made.'
