...
if [ "${WARDROBE_ROLLBACK_TEST:-0}" = '1' ] && [ -n "${WARDROBE_TEST_FORCE_DEPLOY_FAILURE_FILE:-}" ] && [ -f "${WARDROBE_TEST_FORCE_DEPLOY_FAILURE_FILE}" ]; then
  case "${WARDROBE_TEST_FORCE_DEPLOY_FAILURE_FILE}" in
    "$candidates_path"/*|"$BASE_PATH"/.rollback-failure-*) rm -f -- "${WARDROBE_TEST_FORCE_DEPLOY_FAILURE_FILE}" ;;
    *) fail 'Refusing an intentional rollback-test failure outside the isolated test path.' ;;
  esac
...
api_smoke_test "$container_name" || fail 'Wardrobe is healthy but the configured importer is not ready. Check the local API key and model-reference image.'
...
last_backup="$(awk 'NR == 1 { print $2 }' "$backup_path/.last-success")"
...
test "$published_ip" = "$bind_address" || fail 'Container port is not bound to the configured address.'
