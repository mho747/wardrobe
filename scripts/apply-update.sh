...
test "${1:-}" = '--approved-sensitive' || fail 'Refusing update without --approved-sensitive after explicit human approval.'
...
rollback() {
  ...
  if ! ./scripts/deploy-synology.sh; then
    printf '%s\n' 'CRITICAL: automatic rollback could not be verified. Inspect the deployment logs before making further changes.' >&2
    exit 2
  fi
