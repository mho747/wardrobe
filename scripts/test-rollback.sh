...
chmod 700 "$TEST_BASE/data" "$TEST_BASE/backups" "$TEST_BASE/update-state" "$TEST_BASE/candidates" 2>/dev/null || true
printf 'rollback-test-reference\n' > "$TEST_BASE/data/model-reference.png"
chmod 600 "$TEST_BASE/data/model-reference.png" 2>/dev/null || true
...
printf 'Isolated rollback test passed. Git revision %s was restored automatically; no production container, user data, or OpenAI request was used.\n' "$previous_revision"
