...
ROOT="${WARDROBE_DEPLOYMENT_ROOT:-$(CDPATH= cd -- "$(dirname -- "$0")/.." && pwd)}"
CANDIDATES_ROOT="${WARDROBE_CANDIDATES_ROOT:-$ROOT/candidates}"
...
cd "$ROOT"
test -f .env || fail 'Missing .env. Create it from .env.example with secrets stored only on the deployment host.'
...
if [ -n "${WARDROBE_TEST_CANDIDATE_REVISION:-}" ]; then
  [ "${WARDROBE_ROLLBACK_TEST:-0}" = '1' ] || fail 'Refusing a non-GitHub candidate outside an isolated rollback test.'
  [ -n "${WARDROBE_BASE_PATH:-}" ] || fail 'Rollback-test base path is required for a local candidate.'
...
git merge-base --is-ancestor "$current_revision" "$candidate_revision" || fail 'The candidate is not a fast-forward from the deployed revision.'
...
chmod 700 "$candidate_data" "$candidate_backups" "$candidate_state" 2>/dev/null || true
printf '%s\n' \
  'OPENAI_API_KEY=' \
  ...
  "WARDROBE_CANDIDATES_ROOT=$candidate_dir/candidates" \
...
printf 'Candidate passed: %s. No production files, user data, or production containers were changed.\n' "$candidate_revision"
